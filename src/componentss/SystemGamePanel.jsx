...
const explanations = [
  ...
  "The primary route forwards work to Compute. If it disappears, work transfers immediately to Buffer to Standby at the same capacity.",
  ...
  return (
    <section className="system-game" id={panelId} aria-label="Break the system challenge">
      ...
      <div className="game-outage">
        <div><span>{phase === "replaying" ? "Protected replay · 2x" : `Elapsed ${game.time.toFixed(1)} / ${LIMITS.round}s`}</span><span>Outage {game.badFor.toFixed(1)} / {LIMITS.hold}s</span></div>
        ...
        <div className="game-active-faults" aria-label="Active faults">
          {faults.map((id) => (
            <button key={id} type="button" disabled={phase !== "playing"} onClick={() => toggleFault(id)} aria-label={`Remove ${FAULTS.find((fault) => fault.id === id).label}`}>
              {FAULTS.find((fault) => fault.id === id).label}<X size={12} aria-hidden="true" />
            </button>
          ))}
        </div>
      )}
      ...
      {!ended && (
        <div className="game-inspector">
          ...
          {(node === 2 || node === 4) && <button className="game-inline-link" type="button" onClick={() => onSelect(6)}>Inspect pool/shared-01 <ArrowUpRight size={14} aria-hidden="true" /></button>}
      ...
      {ended && (
        <div className={`game-result${phase === "patched" ? " game-result--patched" : ""}`}>
          ...
          {comparison && <div className="game-comparison"><span>Before <strong>{Math.round(comparison.before)}%</strong></span><ArrowRight size={14} aria-hidden="true" /><span>Protected <strong>{Math.round(comparison.after)}%</strong></span></div>}
          {phase === "broken" && <><p className="game-patch-note">The patch: cancel settled or hopeless work, cap queued attempts, and budget retries. Rejected requests still count as failures.</p><button className="game-primary" type="button" onClick={replay}>Patch the design & replay <ArrowUpRight size={14} aria-hidden="true" /></button></>}
          {phase !== "broken" && <button className="game-primary" type="button" onClick={start}>Try another approach <ArrowUpRight size={14} aria-hidden="true" /></button>}
      ...
      <button className="game-telemetry-toggle" type="button" aria-expanded={telemetry} aria-controls={`${panelId}-telemetry`} onClick={onTelemetry}>
        <span>{telemetry ? "Close the notebook" : "Look closer / telemetry & timeline"}</span>{telemetry ? <Minus size={14} aria-hidden="true" /> : <Plus size={14} aria-hidden="true" />}
      </button>
